...
public class AuditLogTicketCreateTestCase23 {
	WebDriver driver;

	public AuditLogTicketCreateTestCase23(WebDriver driver)
	{
		this.driver=driver;
...
//		action.click(system).perform();
	...
	public void verifyTicketCreation()
	{
		...
		auditlogsearch.sendKeys("Link created");
	 ...
	 // Find the first row that contains "Logged"
	    WebElement firstLoggedCell = wait.until(ExpectedConditions.presenceOfElementLocated(
	        By.xpath("//tr[1]/td[contains(text(),'Link created')]")
	    ));
	 ...
	 Assert.assertTrue(isLoggedPresent, "❌ 'Logged' not found in the first row of the Audit Log table!");

	 // If test passes
	 System.out.println("✅ Test Passed: 'Logged' found in the first row of the Audit Log table.");
